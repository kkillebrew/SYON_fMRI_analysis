# HM creating to handle cases with 2 oppPE scans 20230221
...
if ( $#argv > 0 ) then
    set subj = $argv[1]
    set breakPoint = $argv[2] # First scan AFTER the break - HM 20230221 adding ability to handle 2 oppPE scans
else
else
    echo "Please set subject ID and break point (first scan for oppPE_2)"
    exit
endif
...
3dTcat -prefix $output_dir/blip_reverse2 \
    {$data_dir}/oppPE_2.nii.gz # HM 20230221 - Adding to handle 2 oppPEs...
...
set tr_counts = ( 255 255 263 313 313 163 163 163 163 255 255 255 )
...
set breakPoint2dig = (`count -digits 2 $breakPoint $breakPoint`)
3dbucket -prefix gu_base2 pb01.$subj.r$breakPoint2dig.tshift+orig"[2]"
3dcopy gu_base2+orig gu_base2.nii.gz
...
mv fullWarp_abs.nii.gz fullWarp_abs1.nii.gz
mv fullWarp_rel.nii.gz fullWarp_rel1.nii.gz

gradient_unwarp.py gu_base2.nii.gz grad_unwarp2.nii.gz siemens -g                                              \
    /home/shaw-raid1/sponheim-data/python_tools/gradunwarp-1.0.3/coef/3TB_coeff_from_scanner_20200306.grad -n

minn.py makeFullWarpRel -f fullWarp_abs.nii.gz

mv fullWarp_abs.nii.gz fullWarp_abs2.nii.gz
mv fullWarp_rel.nii.gz fullWarp_rel2.nii.gz

set endPoint1 =  `expr $breakPoint - 1`
set first_runs = (`count -digits 2 1 $endPoint1`)
set second_runs = (`count -digits 2 $breakPoint 12`)

# mps 20181019 need to apply now, so we can have rm files with GU...
foreach run ($first_runs)
    3dNwarpApply                             \
        -source pb01.$subj.r$run.tshift+orig \
        -nwarp "fullWarp_rel1.nii.gz"         \
        -prefix rm.$subj.r$run.gu+orig
end

foreach run ($second_runs)
    3dNwarpApply                             \
        -source pb01.$subj.r$run.tshift+orig \
        -nwarp "fullWarp_rel2.nii.gz"         \
        -prefix rm.$subj.r$run.gu+orig
end
...
3dNwarpApply                             \
    -source blip_reverse2+orig            \
    -nwarp "fullWarp_rel2.nii.gz"         \
    -prefix rm.blip_reverse2.gu+orig # HM 20230221


# HM 20230221 - Splitting into 2 streams based on 2 oppPEs, before and after break
# ================================= volreg =================================
...
3dbucket -prefix vr_base2 rm.$subj.r$breakPoint2dig.gu+orig"[2]"
...
foreach run ( $second_runs )
    # register each volume to the base
    3dvolreg -verbose -zpad 1 -base vr_base2+orig                         \
             -1Dfile dfile.r$run.1D -prefix rm.epi.volreg.r$run           \
             -cubic                                                       \
             -1Dmatrix_save mat.r$run.vr.aff12.1D                         \
             rm.$subj.r$run.gu+orig
...
3dvolreg -verbose -zpad 1                \
         -prefix blip_reverse2.vr+orig \
         -cubic                          \
         rm.blip_reverse2.gu+orig
...
3dTcat -prefix blip_forward2.vr rm.epi.volreg.r0$breakPoint+orig"[0..2]"
...
3dTstat -median -prefix rm.blip.med.fwd2 blip_forward2.vr+orig
3dTstat -median -prefix rm.blip.med.rev2 blip_reverse2.vr+orig
...
3dAutomask -apply_prefix rm.blip.med.masked.fwd2 rm.blip.med.fwd2+orig
3dAutomask -apply_prefix rm.blip.med.masked.rev2 rm.blip.med.rev2+orig
...
3dQwarp -plusminus -pmNAMES Rev For                           \
        -pblur 0.05 0.05 -blur -1 -1                          \
        -noweight -minpatch 9                                 \
        -source rm.blip.med.masked.rev2+orig                   \
        -base   rm.blip.med.masked.fwd2+orig                   \
        -prefix blip_warp2
...
3dNwarpApply                         \
    ...
    -prefix align_base
3dNwarpApply                         \
    -source vr_base2+orig             \
    -nwarp "blip_warp2_For_WARP+orig" \
    -prefix align_base2
...
#        mat.r$run.vr.aff12.1D > mat.r$run.warp.1D
    ...
    3dNwarpApply                                                                                                   \
        ...
        -newgrid 2                                                                                                 \
        -nwarp "align_base_al_mat.aff12.1D blip_warp_For_WARP+orig mat.r$run.vr.aff12.1D fullWarp_rel1.nii.gz" \
...
# HM 20230221 - Adding to handle second_runs if 2 oppPEs
foreach run ($second_runs)
     #catenate blip/volreg/epi2anat xforms
#    cat_matvec -ONELINE                          \
#        align_base_al_mat.aff12.1D          \
#        mat.r$run.vr.aff12.1D > mat.r$run.warp.1D
    ...
    3dNwarpApply                                                                                                   \
        ...
        -master anat_SurfVol_ns+orig                                                                               \
        -newgrid 2                                                                                                 \
        -nwarp "align_base2_al2_mat.aff12.1D blip_warp2_For_WARP+orig mat.r$run.vr.aff12.1D fullWarp_rel2.nii.gz" \
        -prefix pb02.$subj.r$run.volreg+orig
# mps 20181019 renaming .align back to .volreg so that afni knows what to do with it later when making out.ss.review
...
foreach run ( $runs ) # e.g., 01, 02...
    ...
    if ( ($run == 01) || ($run == 02) || ($run == 10) || ($run == 11) || ($run == 12) ) then
        set n_TRs = 255
        if ($run == 01) then
