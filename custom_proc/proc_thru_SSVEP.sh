...
# set list of runs; this ppt has no Contour, Size, MTLoc, LOCloc, or retExp
...
# # add regressors
    ...
    else if ( $run == 04 ) then # ss loc
        set n_cond = 2

        cp {$git_regress_dir}/{$task_name_uppercase}_{$condA_name}.1D regressors/{$task_name_uppercase}_{$condA_name}.1D # center
        cp {$git_regress_dir}/{$task_name_uppercase}_{$condB_name}.1D regressors/{$task_name_uppercase}_{$condB_name}.1D # surround
